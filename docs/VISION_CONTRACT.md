...
- Freshness (Agent 3): latest daily snapshot only. Capture current UTC time, use calendar close times for the expected completed session, inspect actual row/window dates. `EligibilityResult.asof_date` is a maximum, not proof of freshness. Fail a globally stale or future snapshot (`StaleSnapshotError`). Record isolated stale candidates as skipped. No arbitrary historical screening cutoff. Polygon OHLC are already adjusted by their source; do not substitute `adj_close` or adjust again. Unknown provenance is the string `"unknown"`.
