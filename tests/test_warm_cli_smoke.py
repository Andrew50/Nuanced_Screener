...
def test_cli_models_warm_noop_smoke(tmp_path: Path) -> None:
    repo_root = tmp_path

    # Provide a bit of history so all sampled asof_date have full windows.
    days = [
        date(2025, 3, 3),
        date(2025, 3, 4),
        date(2025, 3, 5),
    ...
    # labels-only ticker source for warm requires a CSV; content is only used for tickers list.
    ...
    warm_dir = repo_root / "data" / "models" / "dummy_warm_noop" / "_warm"
    warm_files = list(warm_dir.glob("**/warm.json"))
    assert warm_files, f"expected warm.json under {warm_dir}, got none. output={res.output}"
